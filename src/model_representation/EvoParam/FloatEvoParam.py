...
class FloatEvoParam(EvoParam):

    def __init__(self, value):
        assert type(value) is float or value == 0, "value must be of type float"
        assert type(self._value_range) is list, "value_range must be a list"
        assert type(self._value_range[0]) is float, "value_range[0] must be float"
        assert type(self._value_range[1]) is float, "value_range[1] must be float"
        ...
        assert self._mean is not None, f"{self.__class__.__name__}: mean must be set for FloatEvoParams"
        assert self._sd is not None, f"{self.__class__.__name__}: sd must be set for FloatEvoParams"
